...
'with the selected tickerIndex. If it is,
'then assign the current closing price to the tickerEndingPrices variable.
...
If Cells(i - 1, 1).Value = tickers(i) And Cells(i + 1, 1).Value <> tickers(i) Then
'
...
'        Db.Cells(Dc, 1) = i
'    Dc = Dc + 1
'    Db.Cells(Dc, 1) = "ticker ending price"
'    Dc = Dc + 1
'        Db.Cells(Dc, 1) = tickerEndingPrices(tickerIndex)
'    Dc = Dc + 1
    End If

'Write a script that increases the tickerIndex
'if the next row’s ticker doesn’t match the previous row’s ticker.

'3d

' if the next ticker value is different to this row increment
